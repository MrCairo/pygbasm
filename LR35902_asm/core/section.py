...
from .constants import EQU, LBL, STOR, INST, SEC, BRACKETS
...
class SectionParser2:
    ...
    def parse_token(self, token: Token) -> Optional[Section]:
        ...
        if args[1] in "'\"":
            self._label = self._make_string(args[1:])

    def _make_string(self, args: list) -> str:
        begin = args[0]
        end = [i for i, x in enumerate(args) if x is begin]
        if len(end) == 0:
            return None
        return " ".join(args[1:end[0]])


class Section:
    """Handle the parsing of a SECTION line."""
    ...
    def __repr__(self):
        """Return a string for how to re-build this object."""
