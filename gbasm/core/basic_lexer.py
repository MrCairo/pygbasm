...
def is_node_valid(node: dict) -> bool:
    ...
    result = False
    if node:
        if core.DIR in node and TOK in node:
            result = True
    return result
...
def tokenize_line(line: str) -> dict:
    ...
    if clean_split[0] in core.DIRECTIVES:
        tokens[core.DIR] = clean_split[0]
        tokens[TOK] = clean_split
    elif clean_split[0] in core.STORAGE_DIR:
        tokens[core.DIR] = core.STOR
        tokens[TOK] = clean_split
    elif IS().is_mnemonic(clean_split[0]):
        tokens[core.DIR] = core.INST
        tokens[TOK] = clean_split
    elif line[0] in core.LabelUtils.valid_label_first_char():
        if core.LabelUtils.is_valid_label(clean_split[0]):
            tokens[core.DIR] = core.LBL
            data = clean_split
            if len(clean_split) > 1:
                data = [{core.DIR: core.LBL, TOK: clean_split[0]}]
                tokens[core.DIR] = core.MULT
